...
use TelegramRelay\Tests\Doubles\DeferredAdapter;
...
final class StoreAsyncTest extends TestCase
{
    ...
    {
        $this->dir = sys_get_temp_dir() . '/telegramrelay-async-' . bin2hex(random_bytes(6));
    ...
    public function testTheWriteLandsWhenTheBackendFinishes(): void
    {
        ...
        $this->assertSame('-1001', $this->reopen()->links()->telegramFor('chan1'));
    }
    ...
    public function testChangesDuringAWriteCoalesceIntoOneMoreWrite(): void
    {
        ...
        $store->link('guild1', 'chan1', '-1001');   // starts a write
        $store->link('guild1', 'chan2', '-1002');   // queued behind it
        $store->link('guild1', 'chan3', '-1003');   // folded into the same one
